...
class Viewport {
  ...
  /**
   * Creates:
   *  - columnsRenderCalculator (before draw, to qualify columns for rendering) // 在绘制之前，限定要呈现的列
   *  - columnsVisibleCalculator (after draw, to measure which columns are actually visible) // 绘制后，测量哪些列实际上是可见
   *
   * @returns {ViewportRowsCalculator}
   */
