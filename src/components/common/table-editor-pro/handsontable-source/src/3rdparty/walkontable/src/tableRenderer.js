...
class TableRenderer {
  ...
  renderColumnHeaders() {
    ...
    const columnCount = this.wtTable.getRenderedColumnsCount(); // 第一行显示的总列数
    for (let i = 0; i < this.columnHeaderCount; i++) {
      const TR = this.getTrForColumnHeaders(i);
      for (let renderedColumnIndex = (-1) * this.rowHeaderCount; renderedColumnIndex < columnCount; renderedColumnIndex++) {
        const sourceCol = this.columnFilter.renderedToSource(renderedColumnIndex);
        const offsetCol = this.columnFilter.offset;
        this.renderColumnHeader(i, sourceCol, TR.childNodes[renderedColumnIndex + this.rowHeaderCount], offsetCol);
      }
    }
  }
   ...
   * @returns {*}
   */
  renderColumnHeader(row, col, TH, offsetCol) {
    TH.className = '';
    TH.removeAttribute('style');

    return this.columnHeaders[row](col, TH, row, offsetCol);
  }
