...
HandsontableEditor.prototype.createElements = function (...args) {
  ...
  this.elDom = React.createRef();
  const { Option } = Select;
  const _this = this;
  // 渲染内容
  ReactDOM.render(
    <div className="handsontableEditor cho-handsontable-editor" ref={_this.elDom}>
  ...
  this.htContainer = ReactDOM.findDOMNode(this.elDom?.current);
  console.log('htContainer----', this.htContainer);
...
HandsontableEditor.prototype.prepare = function (td, row, col, prop, value, cellProperties, ...args) {
  ...
  console.log('handsontable-------', this.cellProperties.handsontable);
...
HandsontableEditor.prototype.open = function (...args) {
  ...
  console.log('htEditor-----', this.htEditor, this.htContainer);
  this.htEditor.init();
