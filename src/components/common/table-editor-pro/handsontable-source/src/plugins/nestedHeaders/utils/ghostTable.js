...
class GhostTable {
  ...
  /**
   * Build cache of the headers widths. 只在初始化的时候加载了ryp
   *
   * @private
   */
  buildWidthsMapper() {
  ...
  /**
   * Build temporary table for getting minimal columns widths. 只在初始化的时候加载了ryp
   *
   * @private
    ...
    for (let row = 0; row < maxRows; row++) {
      ...
      for (let col = 0; col < maxCols; col++) {
        ...
        if (headerObj && !headerObj.isHidden) {
          ...
          td.rowspan = headerObj?.rowspan; // 增加rowspan ryp
          tr.appendChild(td);
        }
      }
