...
class NestedHeaders extends BasePlugin {
   ...
   * 
   * return中回调函数的参数 renderedColumnIndex, TH, rows, offsetCol
   * renderedColumnIndex 可视列最左侧的下标 最原始的数组中的下标, 从0开始 滑动后隐藏了一列就变成从1开始
   * TH 当前渲染的th
   * rows: 当前是表头中的第几行
   * offsetCol: 偏移量(被隐藏的列数)
   */
  headerRendererFactory(headerLevel) {
    ...
    return (renderedColumnIndex, TH) => {
      ...
      // console.log('h4---' + headerLevel, renderedColumnIndex, TH.innerHTML);
      ...
      // 当前可视列表头配置参数
      const { colspan, label, isHidden, rowspan } = this.#stateManager.getHeaderSettings(headerLevel, visualColumnsIndex); // 增加rowspan ryp

      // TODO:label为空时 隐藏单元格 不是很谨慎 需要再次修改
      if (isHidden === true || !label) {
      ...
      if (rowspan > 1 && headerLevel === 0) {
        TH.setAttribute('rowspan', rowspan); // 增加rowspan ryp
      }
